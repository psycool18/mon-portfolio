<script setup>
    ...
    function sendMail(message) {
        ...
        || message.msg.length < 1 ){
            ...
            let mailto
            if(message.tel) {
                mailto = `mailto:besnard.sylvain@gmx.fr?subject=${message.object}&body=Je%20suis%20%3A%20${message.name}%0AVoici%20mon%20mail%20%3A%20${message.mail}%0AVoici%20mon%20téléphone%20%3A%20${message.tel}%0A%0A${message.msg}`
            }else {
                mailto = `mailto:besnard.sylvain@gmx.fr?subject=${message.object}&body=Je%20suis%20%3A%20${message.name}%0AVoici%20mon%20mail%20%3A%20${message.mail}%0A%0A${message.msg}`
            }
